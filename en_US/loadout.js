function Gear() {
    ...
    this.hasAbility = function(skill) {
        if (this.main != null)
            if (this.main.name == skill) return true;
        for (var i = 0; i < this.subs.length; i++) {
            if (this.subs[i] != null)
                if (this.subs[i].name == skill) return true;
        }
    }
    this.calcAbilityScore = function(skill, mod) {
        var total = 0;
        if (this.main != null)
            if (this.main.name == skill) total += 10;
        for (var i = 0; i < this.subs.length; i++) {
            var value = 3;
            if (this.main != null)
                if (this.main.name == 'Ability Doubler') value = 6;
            if (this.subs[i] != null)
                if (this.subs[i].name == skill) total += value;
        }
        return total;
    }
...
function SplashTag() {
    ...
    this.getTextColor = function() {
        if (this.subject == null) return 'color: rgba(0,0,0, 1)';
        var r = this.bg.color.r * 255;
        var g = this.bg.color.g * 255;
        var b = this.bg.color.b * 255;
        return 'color: rgba(' + r + ',' + g + ',' + b + ', 1)';
    }

    this.getDiscriminator = function() {
        // 0000
        var str = this.discriminator.toString();
        var str = "0000".substr(str.length) + str;
        return "#" + str;
    }

    this.getBadgeIcon = function(x) {
        if (this.badges[x] != null) {
            return this.badges[x].image
        } else return '../common/assets/img/skills/Unknown.png'
    };
}
...
function Loadout() {
    ...
    this.setOpenSlot = function(skill) {
        ...
        if (this.head.hasEmptySlots()) { this.head.setNextSlot(skill); return true; } else if (this.clothes.hasEmptySlots()) { this.clothes.setNextSlot(skill); return true; } else if (this.shoes.hasEmptySlots()) { this.shoes.setNextSlot(skill); return true; }
        return false;
    }
    ...
    this.swapSlots = function(s1, s2) {
        ...
        if (slot1Value != null)
            if (slot1Value.exclusive) return;
        if (slot2Value != null)
            if (slot2Value.exclusive) return;
        var tmp = slot1Value;
